        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                timeout=aiohttp.ClientTimeout(
                    total=30
                ),  # Increased from 5s for network reliability
            ) as resp:
                if resp.status == 200:
                    logger.info(
                    ...
                    logger.error(
                        f"🔴 Position sync failed (HTTP {resp.status}): {error_text}"
                    )
                    raise HTTPException(
                        status_code=500,
                        detail=f"Backup position sync failed: {error_text}",
                    )
...
async def receive_position_from_primary(request: Request):
    ...
    try:
        ...
        logger.error(
            f"🔴 Position reception failed: {type(e).__name__}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=500, detail=f"Position reception failed: {str(e)}"
        )
