...
async def get_postgresql_replication_lag():
    ...
    try:
        ...
        status = "healthy" if lag_seconds < 2 else "warning" if lag_seconds < 5 else "critical"
...
async def get_postgresql_status():
    ...
    try:
        ...
        return JSONResponse({
            ...
            "wal": {
                "current_lsn": str(wal_lsn)
            },
            "replication": {
                "replica_count": int(replica_count) if replica_count else 0,
                "max_lag_seconds": round(max_lag, 2),
                "status": "healthy" if max_lag < 2 else "warning" if max_lag < 5 else "critical"
            }
        })
