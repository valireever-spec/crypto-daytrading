    ...
    return JSONResponse(
        {
            ...
            "daily_start_value": monitor.daily_start_value
            or account.get("total_equity", 0),
...
async def get_concentration_risk():
    ...
    if not account or not account.get("positions"):
        return JSONResponse(
            {"concentration_hhi": 0.0, "message": "No positions", "risk_level": "green"}
        )
    ...
    return JSONResponse(
        {
            ...
            "largest_position_pct": (max(weights.values()) / total_value * 100)
            if weights
            else 0,
            "risk_level": risk_level,
            "recommendation": "Diversify further"
            if hhi > 0.15
            else "Concentration risk is acceptable",
        }
    )
    ...
    return JSONResponse(
        {
            ...
            if allowed
            else "Position exceeds maximum size limit",
        }
    )
