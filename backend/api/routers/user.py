...
async def update_user_settings(settings: dict) -> JSONResponse:
    ...
    if "auto_refresh_interval" in settings and isinstance(
        settings["auto_refresh_interval"], int
    ):
    ...
    if "preferred_currencies" in settings and isinstance(
        settings["preferred_currencies"], list
    ):
